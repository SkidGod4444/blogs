        ...
        return (
          <li key={index} className={cn("mt-0 relative")}>
            {level !== 1 && (
              <div 
                className={cn(
                  ...
                  (isActive || isParentActive) ? "border-border" : "border-muted"
                )}
                style={{
            ...
            <button
              ...
              className={cn(
                "w-full text-left py-2 px-4 relative rounded hover:text-secondary-foreground cursor-pointer transition-all duration-300 ease-in-out",
                isActive ? "text-secondary-foreground font-medium" : "text-muted-foreground"
              )}
            >
              {isActive && (
                <span className="bg-secondary-foreground rounded-xl h-[30px] w-0.5 absolute left-0 top-0" />
              )}
              {item.title}
            </button>
            {item.items?.length ? (
